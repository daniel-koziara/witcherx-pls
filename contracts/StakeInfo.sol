...
//custom errors
error WitcherX_InvalidStakeLength();
error WitcherX_RequireOneMinimumShare();
error WitcherX_ExceedMaxAmountPerStake();
error WitcherX_NoStakeExists();
error WitcherX_StakeHasEnded();
error WitcherX_StakeNotMatured();
error WitcherX_StakeHasBurned();
error WitcherX_MaxedWalletStakes();
...
abstract contract StakeInfo {
    ...
    /** @dev track global staked witcher */
    uint256 private s_globalWitcherStaked;
    ...
    function _startStake(
        ...
        if (sId > MAX_STAKE_PER_WALLET) revert WitcherX_MaxedWalletStakes();
        if (numOfDays < MIN_STAKE_LENGTH || numOfDays > MAX_STAKE_LENGTH)
            revert WitcherX_InvalidStakeLength();

        //calculate shares
        uint256 shares = calculateShares(amount, numOfDays, shareRate);
        if (shares / SCALING_FACTOR_1e18 < 1) revert WitcherX_RequireOneMinimumShare();
     ...
     * @param isPayoutTriggered has global payout triggered
     * @return witcher witcher principle
     */
    function _endStake(
        address user,
        uint256 id,
        ...
        PayoutTriggered isPayoutTriggered
    ) internal returns (uint256 witcher) {
        uint256 globalStakeId = s_addressSIdToGlobalStakeId[user][id];
        if (globalStakeId == 0) revert WitcherX_NoStakeExists();

        UserStakeInfo memory userStakeInfo = s_globalStakeIdToStakeInfo[globalStakeId];
        if (userStakeInfo.status == StakeStatus.ENDED) revert WitcherX_StakeHasEnded();
        if (userStakeInfo.status == StakeStatus.BURNED) revert WitcherX_StakeHasBurned();
        ...
        if (payOther == StakeAction.END_OTHER && block.timestamp < userStakeInfo.maturityTs)
            revert WitcherX_StakeNotMatured();

        //update shares changes
        uint256 shares = userStakeInfo.shares;
        _updateSharesStats(user, shares, userStakeInfo.witcherAmount, day, isPayoutTriggered, action);
        ...
        witcher = _calculatePrinciple(user, globalStakeId, userStakeInfo, action);
    }
    ...
    function _calculatePrinciple(
        ...
        uint256 witcherAmount = userStakeInfo.witcherAmount;
        ...
        uint256 penaltyAmount;
        penaltyAmount = (witcherAmount * penalty) / 100;
        principle = witcherAmount - penaltyAmount;
    ...
    /** @notice get total witcher staked
     * @return totalWitcherStaked total witcher staked
     */
    function getTotalWitcherStaked() public view returns (uint256) {
        return s_globalWitcherStaked;
    }
