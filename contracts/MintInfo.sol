...
//custom errors
error WitcherX_InvalidMintLength();
error WitcherX_InvalidMintPower();
error WitcherX_NoMintExists();
error WitcherX_MintHasClaimed();
error WitcherX_MintNotMature();
error WitcherX_MintHasBurned();
...
abstract contract MintInfo {
    ...
    /** @dev track total witcher minting */
    uint256 private s_globalWitcherMinting;
    /** @dev track total witcher penalty */
    uint256 private s_globalWitcherMintPenalty;
    ...
    struct UserMintInfo {
        uint8 mintPower;
        uint16 numOfDays;
        uint96 mintableWitcher;
        uint48 mintStartTs;
        uint48 maturityTs;
        uint32 mintPowerBonus;
        uint32 EAABonus;
        uint128 mintedWitcher;
        uint64 mintCost;
        MintStatus status;
    }
     ...
     * @param mintableWitcher mintable witcher
    ...
    function _startMint(
        ...
        mintable = calculateMintReward(mintPower, numOfDays, mintableWitcher, EAABonus, burnAmpBonus);
        ...
        UserMintInfo memory userMintInfo = UserMintInfo({
            ...
            mintableWitcher: uint96(mintable),
    ...
    function _startBatchMint(
        ...
        uint256 gMinting = s_globalWitcherMinting;
    ...
    function _updateMintStats(uint256 currentTRank, uint256 gMintPower, uint256 gMinting) internal {
        ...
        s_globalWitcherMinting = gMinting;
    }
    ...
    function _claimMint(
        ...
        if (tRank == 0) revert WitcherX_NoMintExists();

        UserMintInfo memory mint = s_tRankToMintInfo[tRank];
        if (mint.status == MintStatus.CLAIMED) revert WitcherX_MintHasClaimed();
        if (mint.status == MintStatus.BURNED) revert WitcherX_MintHasBurned();
        ...
        if (mint.maturityTs > block.timestamp && action == MintAction.CLAIM)
            revert WitcherX_MintNotMature();

        s_globalWitcherMinting -= mint.mintableWitcher;
    ...
    function _calculateClaimReward(
        ...
        reward = uint256(userMintInfo.mintableWitcher) + (bonus / SCALING_FACTOR_1e7);
        ...
        if (penaltyAmount != 0) s_globalWitcherMintPenalty += penaltyAmount;

        //only stored minted amount for claim mint
        if (action == MintAction.CLAIM) s_tRankToMintInfo[tRank].mintedWitcher = uint128(reward);
    ...
    /** @notice Return total minting witcher
     * @return totalMinting total minting witcher
     */
    function getTotalMinting() public view returns (uint256) {
        return s_globalWitcherMinting;
    }

    /** @notice Return total witcher penalty
     * @return totalWitcherPenalty total witcher penalty
     */
    function getTotalMintPenalty() public view returns (uint256) {
        return s_globalWitcherMintPenalty;
    }
}
