...
import "../interfaces/IWitcherxOnBurn.sol";
...
//custom errors
error WitcherX_InvalidAmount();
error WitcherX_InsufficientBalance();
error WitcherX_NotSupportedContract();
error WitcherX_InsufficientProtocolFees();
error WitcherX_FailedToSendAmount();
error WitcherX_NotAllowed();
error WitcherX_NoCycleRewardToClaim();
error WitcherX_NoSharesExist();
error WitcherX_EmptyUndistributeFees();
error WitcherX_InvalidBurnRewardPercent();
error WitcherX_InvalidBatchCount();
error WitcherX_InvalidMintLadderInterval();
error WitcherX_InvalidMintLadderRange();
error WitcherX_MaxedWalletMints();
error WitcherX_LPTokensHasMinted();
error WitcherX_InvalidAddress();
error WitcherX_InsufficientBurnAllowance();

/** @title WitcherX */
contract TESTX is ERC20, ReentrancyGuard, GlobalInfo, MintInfo, StakeInfo, BurnInfo, OwnerInfo {
    /** Storage Variables*/
    ...
    constructor(address genesisAddress, address buyAndBurnAddress) ERC20("TEST X", "TESTX") {
        if (genesisAddress == address(0)) revert WitcherX_InvalidAddress();
        if (buyAndBurnAddress == address(0)) revert WitcherX_InvalidAddress();
    ...
    function startMint(
        ...
        if (getUserLatestMintId(_msgSender()) + 1 > MAX_MINT_PER_WALLET)
            revert WitcherX_MaxedWalletMints();
    ...
    function batchMint(
        ...
        if (count == 0 || count > MAX_BATCH_MINT_COUNT) revert WitcherX_InvalidBatchCount();
    ...
    function setBuyAndBurnContractAddress(address contractAddress) external onlyOwner {
        if (contractAddress == address(0)) revert WitcherX_InvalidAddress();
    ...
    function setNewGenesisAddress(address newAddress) external {
        if (_msgSender() != s_genesisAddress) revert WitcherX_NotAllowed();
        if (newAddress == address(0)) revert WitcherX_InvalidAddress();
    ...
    function mintLPTokens() external {
        if (_msgSender() != s_buyAndBurnAddress) revert WitcherX_NotAllowed();
        if (s_initialLPMinted == InitialLPMinted.YES) revert WitcherX_LPTokensHasMinted();
        s_initialLPMinted = InitialLPMinted.YES;
        _mint(s_buyAndBurnAddress, INITAL_LP_TOKENS);
    }

    /** @notice burn all BuyAndBurn contract witcher */
    ...
    function _protocolFees(uint256 mintPower, uint256 count) private {
        ...
        if (msg.value < protocolFee) revert WitcherX_InsufficientProtocolFees();
    ...
    /** @dev burn liquid Witcher through other project.
     * called by other contracts for proof of burn 2.0 with up to 8% for both builder fee and user rebate
     * @param user user address
     * @param amount liquid witcher amount
     * @param userRebatePercentage percentage for user rebate in liquid witcher (0 - 8)
     * @param rewardPaybackPercentage percentage for builder fee in liquid witcher (0 - 8)
     * @param rewardPaybackAddress builder can opt to receive fee in another address
     */
    function _burnLiquidWitcher(
        address user,
        uint256 amount,
        ...
        if (amount == 0) revert WitcherX_InvalidAmount();
        if (balanceOf(user) < amount) revert WitcherX_InsufficientBalance();
    ...
    function _burnbefore(
        ...
        if (rewardPaybackPercentage + userRebatePercentage > MAX_BURN_REWARD_PERCENT)
            revert WitcherX_InvalidBurnRewardPercent();
        ...
        if (
            ...
            !IERC165(_msgSender()).supportsInterface(type(IWitcherxOnBurn).interfaceId)
        ) revert WitcherX_NotSupportedContract();
    }
    ...
    function _burnAfter(
        ...
        IWitcherxOnBurn(_msgSender()).onBurn(user, amount);
    }
    ...
    function _sendViaCall(address payable to, uint256 amount) private {
        if (to == address(0)) revert WitcherX_InvalidAddress();
        (bool sent, ) = to.call{value: amount}("");
        if (!sent) revert WitcherX_FailedToSendAmount();
    }
    ...
    function _spendBurnStakeAllowance(address user) private {
        ...
        if (currentAllowance != type(uint256).max) {
            if (currentAllowance == 0) revert WitcherX_InsufficientBurnAllowance();
    ...
    /** @notice get user current cycle total witcher burned
     * @param user user address
    ...
    //Public functions for devs to intergrate with Witcher
    /** @notice allow anyone to sync dailyUpdate manually */
    function manualDailyUpdate() public dailyUpdate {}

    /** @notice Burn Witcher tokens and creates Proof-Of-Burn record to be used by connected DeFi and fee is paid to specified address
     * @param user user address
     * @param amount witcher amount
     * @param userRebatePercentage percentage for user rebate in liquid witcher (0 - 8)
     * @param rewardPaybackPercentage percentage for builder fee in liquid witcher (0 - 8)
    ...
    function burnTokens(
        ...
        _burnLiquidWitcher(user, amount, userRebatePercentage, rewardPaybackPercentage, _msgSender());
    }

    /** @notice allows user to burn liquid witcher directly from contract
     * @param amount witcher amount
     */
    ...
    function approveBurnMints(address spender, uint256 amount) public returns (bool) {
        if (spender == address(0)) revert WitcherX_InvalidAddress();
