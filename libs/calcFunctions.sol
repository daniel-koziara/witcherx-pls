 ...
 * @param mintableWitcher current contract day mintable witcher
 ...
 * @return reward base witcher amount
 */
function calculateMintReward(
    ...
    uint256 baseReward = (mintableWitcher * mintPower * numOfDays);
    if (numOfDays != 1)
 ...
 * @return bonus bonus amount in witcher
 */
...
error WitcherX_AtLeastHalfMaturity();
...
function calculateShareBonus(uint256 amount, uint256 noOfDays) pure returns (uint256 shareBonus) {
    ...
    uint256 cappedStakedWitcher = amount <= BPB_MAX_WITCHER ? amount : BPB_MAX_WITCHER;
    shareBonus =
        ((cappedExtraDays * SCALING_FACTOR_1e11) / LPB_PER_PERCENT) +
        ((cappedStakedWitcher * SCALING_FACTOR_1e11) / BPB_PER_PERCENT);
    return shareBonus;
}
...
function calculateEndStakePenalty(
    ...
    if (block.timestamp < stakeStartTs + (maturityTs - stakeStartTs) / 2)
        revert WitcherX_AtLeastHalfMaturity();
